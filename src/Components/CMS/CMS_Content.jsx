...
const CMSDashboard = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    collegeName: '', yearOfStudying: '', courseOpt: '',
    registeredMonth: '', projectedAmount: '', preRegisteredAmount: ''
  });
  ...
  // Add filtered executives calculation
  const filteredAssignExecutives = executives.filter(exec => 
    exec.username.toLowerCase().includes(assignSearch.toLowerCase()) ||
    exec.email.toLowerCase().includes(assignSearch.toLowerCase())
  );
  ...
  const columns = [
    ...
    { id: 'yearOfStudying', label: 'Year of Studying' },
    { id: 'courseOpt', label: 'Course Opted' },
    ...
    try {
      ...
      setEntries(
        data.data.map(entry => ({
          ...
          yearOfStudying: entry.yearOfStudying ? String(entry.yearOfStudying) : '',
          courseOpt: entry.courseOpt || '',
          registeredMonth: entry.registeredMonth || '',
          projectedAmount: entry.projectedAmount ? String(entry.projectedAmount) : '',
          preRegisteredAmount: entry.preRegisteredAmount ? String(entry.preRegisteredAmount) : ''
        }))
      );
      setLoading(false);
    } catch (err) {
  ...
  const openEditModal = (entry) => {
    ...
    if (entry.assignedTo !== user.email &&
      user.role !== 'Admin' &&
      user.mainPosition !== 'Product Manager') {
      ...
      <thead>
        <tr className="bg-gray-100">
          ...
          {visibleColumns.actions && (
            <th className="px-2 py-1 border text-xs font-medium">
  ...
  const handleBulkAssign = async () => {
    if (!selectedExecutive || selectedEntries.size === 0) return;
    ...
    try {
      ...
      // Get current entries data first
      const entriesToUpdate = entries.filter(entry => 
        selectedEntries.has(entry.id)
      );

      // Update each selected entry with merged data
      await Promise.all(
      ...
      toast.success(`Successfully assigned ${selectedEntries.size} entries!`);
      setSelectedEntries(new Set());
      fetchEntries();
    } catch (err) {
      console.error('Bulk assign error:', err);
      toast.error(`Failed to assign entries: ${err.message}`);
    }
  };
  ...
  return (
    ...
    ${disabledFields.contact ? 'bg-gray-100 cursor-not-allowed' : ''}`}
      ...
      ${disabledFields.assignedTo || user.mainPosition === "Executive" ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                    ...
                    <div className='grid grid-col-1 sm:grid-col-2 gap-4'>
                      {/* College Name */}
                      <div>
                      ...
                      {/* Year of Studying */}
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Year of Studying</label>
                        <input
                          type="number"
                          value={formData.yearOfStudying}
                          onChange={(e) => setFormData({ ...formData, yearOfStudying: e.target.value })}
                          className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500"
                      ...
                      {/* Registered Month */}
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Registered Month</label>
                        <input
                          type="month"
                          value={formData.registeredMonth}
                          onChange={(e) => setFormData({ ...formData, registeredMonth: e.target.value })}
                          className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500"
                        />
                      </div>
                      ...
                      {/* Projected Amount */}
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Projected Amount</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Pre-Registered Amount</label>
                        <input
                          type="number"
                          value={formData.preRegisteredAmount}
                          onChange={(e) => setFormData({ ...formData, preRegisteredAmount: e.target.value })}
                          className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500"
                          placeholder="Enter Pre-Registered Amount"
                        />
                      </div>
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute bottom-2 right-2 flex items-center gap-2">
                          ...
                          <label
                            ...
                            className={`cursor-pointer p-1 rounded ${isUploadingImage ? 'bg-gray-200' : 'hover:bg-gray-100'}`}
                          >
                            {isUploadingImage ? (
            ...
            {currentComments.length > 0 ? (
                <div className="space-y-3">
                    {currentComments.map((comment, index) => {
                        const isImage = comment.comment.startsWith("[image:");
                        const imageUrl = isImage ? comment.comment.replace("[image:", "").replace("]", "") : null;
    ...
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      {selectedEntries.size > 0 && (
        <button
          ...
          className="bg-green-500 text-white py-1 px-2 rounded flex items-center relative top-4 ml-2"
        >
